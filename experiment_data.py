...
class ExperimentDataCollector:
    ...
    def _generate_experiment_report(self):
        ...
        # 将所有NaN/Inf清洗为可序列化的安全数值
        report = self._sanitize_numeric(report)
    ...
    def _sanitize_numeric(self, obj):
        """递归替换NaN/Inf为0，保证报告中没有非法数值"""
        if isinstance(obj, float):
            if np.isnan(obj) or np.isinf(obj):
                return 0.0
            return obj
        if isinstance(obj, dict):
            return {k: self._sanitize_numeric(v) for k, v in obj.items()}
        if isinstance(obj, list):
            return [self._sanitize_numeric(v) for v in obj]
        return obj
