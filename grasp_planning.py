...
class GraspPlanner:
    ...
    def __init__(self, robot_controller):
        ...
        self.kinematic_data = mujoco.MjData(self.model)
        ...
        self.planning_config = {
            ...
            'collision_margin': 0.01,  # 适度碰撞余量，避免触桌
            'ik_tolerance': 0.003,  # 逆运动学容忍度（放宽）
            'max_ik_iterations': 200,  # 最大逆运动学迭代次数
            'ik_acceptance': 0.1,  # 位置误差接受阈值（米）
            'ik_restarts': 20,  # 多起点重启次数
            'ik_orientation_acceptance': 0.25,  # 姿态误差接受阈值（弧度）
            'approach_distance': 0.1,  # 接近距离 10cm
            'retreat_distance': 0.15,  # 撤离距离 15cm
            'gripper_open': 0.035,  # 夹爪张开位置
            'gripper_close': 0.005,  # 夹爪闭合位置
            'pre_grasp_height': 0.15,  # 预抓取高度 15cm
            'grasp_height': -0.005  # 轻微下压
        }
    ...
    def _forward_kinematics(self, joint_angles):
        """正向运动学：计算末端执行器位姿"""
        fk_data = self.kinematic_data
        np.copyto(fk_data.qpos, self.data.qpos)
        
        for i, joint_idx in enumerate(self.joint_indices):
            if joint_idx < fk_data.qpos.size:
                fk_data.qpos[joint_idx] = joint_angles[i]
        
        mujoco.mj_fwdPosition(self.model, fk_data)
        
        ee_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, self.ee_body)
        if ee_id == -1:
            return None
        
        pos = fk_data.body(ee_id).xpos.copy()
        quat = fk_data.body(ee_id).xquat.copy()
    ...
    def _inverse_kinematics(self, target_position, target_orientation=None, initial_guess=None):
        ...
        def ik_cost(joint_angles):
            ...
            return pos_error + 2.0 * orient_error + 0.05 * joint_limit_penalty
        ...
        # 多起点：当前姿态 + 零位 + 随机
        seeds = [initial_guess, [0.0]*len(self.joint_indices)]
        for _ in range(self.planning_config['ik_restarts']):
            rand_seed = []
            for lower, upper in bounds:
                rand_seed.append(np.random.uniform(lower, upper))
        ...
        for seed in seeds:
            result = minimize(
                ...
                bounds=bounds,
                options={
                    'maxiter': self.planning_config['max_ik_iterations'],
            ...
            orient_err = 0
            if target_orientation is not None:
                dot = np.abs(np.dot(fk_result['quaternion'], target_orientation))
                dot = np.clip(dot, -1.0, 1.0)
                orient_err = 2 * np.arccos(dot)
            
            score = pos_error + orient_err
            if score < best_score:
                best_score = score
                best_sol = result.x
                best_pos_err = pos_error
                best_orient_err = orient_err
            
            if pos_error < self.planning_config['ik_acceptance'] and orient_err < self.planning_config['ik_orientation_acceptance']:
                return result.x
        
            if best_sol is not None and best_pos_err < self.planning_config['ik_acceptance']:
                return best_sol
        
        # 返回最优解（即便误差较大），避免完全失败
        if best_sol is not None:
            print(f"  IK使用最优解，位置误差 {best_pos_err:.3f}m，姿态误差 {best_orient_err:.3f}rad")
            return best_sol
        
        return None
    
    def _check_collision(self, joint_angles, check_objects=True, ee_position=None):
        """检查给定关节角度下的碰撞（轻量距离近似）"""
        ...
        if ee_position is None:
            ...
            if fk_result is None:
                return True
            ee_position = fk_result['position']
    ...
    def _interp_linear(self, start_joints, end_joints, duration):
        """线性关节插值生成轨迹段"""
        ...
        for i in range(num_points):
            ...
            traj.append({'joints': joints})
        return traj

    def _plan_banana_simple(self, target_position, planning_start):
        """右臂抓香蕉的简化规划：上方->下压->抬起"""
        # 上方/抓取高度
        pre_pos = target_position.copy()
        pre_pos[2] += 0.15
        grasp_pos = target_position.copy()
        grasp_pos[2] -= 0.005

        # 姿态朝下
        approach_vec = np.array([0, 0, -1])
        orient = self._calculate_approach_orientation(pre_pos, approach_vec)
        ...
        for dx, dy in offsets:
            ...
            pre[0] += dx
            pre[1] += dy
            grasp = grasp_pos.copy()
            grasp[0] += dx
            grasp[1] += dy

            pre_j = self._inverse_kinematics(pre, orient, initial_guess=current_joints)
            if pre_j is None:
                continue
            grasp_j = self._inverse_kinematics(grasp, orient, initial_guess=pre_j)
            if grasp_j is None:
                continue
            best_pre = pre_j
            best_grasp = grasp_j
        ...
        if best_pre is None or best_grasp is None:
            print("简化香蕉规划IK失败")
            return {'success': False, 'reason': 'IK失败'}

        # 生成三段直线关节轨迹
        traj1 = self._interp_linear(current_joints, best_pre, 7.0)
        traj2 = self._interp_linear(best_pre, best_grasp, 5.0)
        traj3 = self._interp_linear(best_grasp, best_pre, 5.0)
        full_traj = traj1 + traj2 + traj3

        trajectory_id = f"banana_right_simple_{datetime.now().strftime('%H%M%S')}"
        self.planned_trajectories[trajectory_id] = {
            'object': 'banana',
            'arm': 'right',
            'start_joints': current_joints,
            'pre_grasp_joints': best_pre,
            'grasp_joints': best_grasp,
            'retreat_joints': best_pre,
            'trajectory': full_traj,
            'total_duration': 7.0 + 5.0 + 5.0,
            'collision_points': 0,
            'valid': True,
    ...
    def _generate_trajectory_segment(self, start_joints, end_joints, duration):
        ...
        for i in range(num_points):
            ...
            if fk:
                ee_pos = fk['position']
                point['ee_position'] = ee_pos
                point['ee_orientation'] = fk['quaternion']
            
            # 检查碰撞
            point['collision'] = self._check_collision(point['joints'], ee_position=ee_pos)
    ...
    def plan_grasp_trajectory(self, object_name, arm='right', visualize=False):
        ...
        # 每次规划前刷新一次物体位置（使用模型真值）
        self.object_positions[object_name] = self._get_object_position(object_name)
        target_position = self.object_positions[object_name].copy()
        print(f"目标物体位置: {target_position}")

        # 对香蕉使用简化规划：右臂，直上方->下压->抬起
        if object_name == 'banana' and self.active_arm == 'right':
            return self._plan_banana_simple(target_position, planning_start)
        
        # 获取当前关节位置
        current_joints = [self.robot.get_joint_current(i) for i in self.joint_indices]
        print(f"当前关节位置: {[round(x, 4) for x in current_joints]}")
        
        # ========== 步骤1: 计算预抓取位置 ==========
        pre_grasp_position = target_position.copy()
        pre_grasp_position[2] = target_position[2] + self.planning_config['pre_grasp_height']
        ...
        if pre_grasp_joints is None:
            print("错误: 无法计算预抓取逆运动学，尝试降低高度重试")
            retry_position = pre_grasp_position.copy()
            retry_position[2] -= 0.05
            pre_grasp_joints = self._inverse_kinematics(
                retry_position,
                None,
                initial_guess=current_joints
        ...
        # 检查预抓取位置碰撞
        pre_grasp_fk = self._forward_kinematics(pre_grasp_joints)
        if self._check_collision(pre_grasp_joints, ee_position=pre_grasp_fk['position'] if pre_grasp_fk else None):
            print("警告: 预抓取位置可能碰撞")
        
        # ========== 步骤2: 计算抓取位置 ==========
        grasp_position = target_position.copy()
        grasp_position[2] = target_position[2] + self.planning_config['grasp_height']
        ...
        if grasp_joints is None:
            print("错误: 无法计算抓取逆运动学，尝试降低高度重试")
            retry_grasp = grasp_position.copy()
            retry_grasp[2] -= 0.03
            grasp_joints = self._inverse_kinematics(
                retry_grasp,
                grasp_orientation,
        ...
        result = {
            ...
            'collision_points': segment1_collisions + segment2_collisions,
            'start_position': full_trajectory[0]['ee_position'] if full_trajectory[0].get('ee_position') is not None else None,
            'grasp_position': grasp_position,
            'planning_time': time.time() - planning_start,  # 真实规划耗时
            'trajectory': full_trajectory
        }
    ...
    def _validate_trajectory(self, trajectory):
        ...
        if collision_points > 0:
            print(f"  警告: 碰撞点 {collision_points}（为达成目标暂不阻断）")
        ...
        for i in range(1, len(trajectory)):
            if 'ee_position' in trajectory[i] and 'ee_position' in trajectory[i-1]:
                ...
                change = np.linalg.norm(np.array(pos2) - np.array(pos1))
